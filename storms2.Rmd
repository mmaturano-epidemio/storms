---
title: "Quantifying the Global Burden of Severe Weather Events: A Unified Analysis of Health and Economic Loss (1993-2011)"
subtitle: "A unified analysis of mortality, morbidity, and economic loss (1993-2011)"
output:
  html_document:
    toc: true
    toc_float: true
...
# Results
Between the years `r tidy_storms[, min(year)]` and `r tidy_storms[, max(year)]`, the NOAA database captured a total of `r format(tidy_storms[, .N], big.mark=",")` extreme weather events. These disasters resulted in `r format(tidy_storms[, sum(fatalities)], big.mark=",")` fatalities and `r format(tidy_storms[, sum(injuries)], big.mark=",")` injuries. Beyond the human toll, the economic impact was staggering, with an estimated $`r round(tidy_storms[, sum(economic_loss)] / 10^9, 2)` billion in property and crop damage.
...
```{r  impact_heatmap, fig.height = 12, fig.width = 10, fig.cap = "Heatmap of Weather Impact"}
...
- South: `r (tidy_storms[region == "South", sum(economic_loss)] / 10^9) |> round(2)` B

- West: `r (tidy_storms[region == "West", sum(economic_loss)] / 10^9) |> round(2)` B

- Midwest: West: `r (tidy_storms[region == "Midwest", sum(economic_loss)] / 10^9) |> round(2)` B

- Northeast: $15.32 `r (tidy_storms[region == "Northeast", sum(economic_loss)] / 10^9) |> round(2)` B
